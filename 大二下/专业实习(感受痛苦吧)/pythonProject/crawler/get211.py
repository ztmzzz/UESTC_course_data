...
while schoolId < 4000:
    ...
    try:
        ...
        if flag:
            ...
            driver.get(url)
            with open('done211.txt', 'a+', encoding='utf-8') as f:
    ...
    for i in ["provinceScore", "plane", "subjectScore"]:  # 恢复时使用
        ...
        while True:  # 省份遍历
                ...
                while True:  # 文理科遍历
                    if i != "provinceScore":
                                ...
                                while True:  # 专业组遍历
                                    while True:  # 处理多页
                                        try:
                                            ...
                                            sleep(2)
                                            data = getData(bodyXpath[dict[i]])
                                        writeData(data, i + "211.txt", dict[i], nowProvince, nowYear, nowWenli,
                                                  nowBatch,
                        ...
                        try:
                            ...
                            data = getData(bodyXpath[dict[i]])
                        if nowProvince in ["北京", "天津", "上海", "江苏", "福建", "湖北", "湖南", "广东", "海南"]:
                            writeData(data, i + "211.txt", dict[i], nowProvince, nowYear, nowWenli, "", "", school,
                                      True)
                        else:
                            writeData(data, i + "211.txt", dict[i], nowProvince, nowYear, nowWenli, "", "", school,
                                      False)
            ...
            if nowProvince == startProvince:
                break
    with open('finish211.txt', 'a+', encoding='utf-8') as f:
        f.write(str(schoolId) + '\n')
    schoolId += 1
