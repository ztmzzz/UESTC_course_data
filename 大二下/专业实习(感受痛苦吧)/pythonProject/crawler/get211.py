...
while schoolId < 4000:
    ...
    driver.get(url)
    sleep(2)
    with open('done211.txt', 'r', encoding='utf-8') as f:
        done = f.readlines()
        if str(schoolId) + '\n' in done:
            schoolId = int(done[-1]) + 1
            continue
            # pass
    try:
        flag = False
        ben = driver.find_elements_by_class_name('line2_item')
        for xxx in ben:
            if xxx.text == '普通本科':
                flag = True
            if xxx.text == '985工程':
        ...
        if flag:
            ...
            driver.get(url)
            with open('普通本科.txt', 'a+', encoding='utf-8') as f:
    ...
    for i in ["provinceScore", "plane", "subjectScore"]:  # 恢复时使用
        ...
        while True:  # 省份遍历
                ...
                while True:  # 文理科遍历
                    if i != "provinceScore":
                            ...
                            if (nowProvince in ["北京", "天津", "上海", "江苏", "福建", "湖北", "湖南", "广东", "海南"]) & (
                                ...
                                while True:  # 专业组遍历
                                    while True:  # 处理多页
                                        try:
                                            ...
                                            sleep(2)
                                            data = getData(bodyXpath[dict[i]])
                                        writeData(data, i + "111.txt", dict[i], nowProvince, nowYear, nowWenli,
                                                  nowBatch,
                                ...
                                while True:  # 处理多页
                                    try:
                                        ...
                                        sleep(2)
                                        data = getData(bodyXpath[dict[i]])
                                    writeData(data, i + "111.txt", dict[i], nowProvince, nowYear, nowWenli, nowBatch,
                                              "",
                                              school,
                                              False)
                                    if isElementExist(nextButtonXpath[dict[i]]):
                        ...
                        try:
                            ...
                            data = getData(bodyXpath[dict[i]])
                        if nowProvince in ["北京", "天津", "上海", "江苏", "福建", "湖北", "湖南", "广东", "海南"]:
                            writeData(data, i + "111.txt", dict[i], nowProvince, nowYear, nowWenli, "", "", school,
                                      True)
                        else:
                            writeData(data, i + "111.txt", dict[i], nowProvince, nowYear, nowWenli, "", "", school,
                                      False)
            ...
            if nowProvince == startProvince:
                break
    with open('finishbenke.txt', 'a+', encoding='utf-8') as f:
        f.write(str(schoolId) + '\n')
    schoolId += 1
