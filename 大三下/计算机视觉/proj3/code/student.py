...
def get_tiny_images(image_paths):
    ...
    for i, image_path in enumerate(image_paths):
        ...
        image = resize(image, (16, 16)).flatten()
        tiny_images[i] = image / norm(image)
    return tiny_images
...
def build_vocabulary(image_paths, vocab_size):
    ...
    '''
    feature_vector_images = []
    for image_path in image_paths:
        image = imread(image_path)
        feature = skimage.feature.hog(image, feature_vector=True, pixels_per_cell=(4, 4), cells_per_block=(2, 2))
        feature = feature.reshape(-1, 2 * 2 * 9)
        feature_vector_images.append(feature)
    feature_vectors = np.vstack(feature_vector_images)
    kmeans = MiniBatchKMeans(n_clusters=vocab_size, max_iter=200).fit(feature_vectors)
    res = np.vstack(kmeans.cluster_centers_)
    return res
...
def get_bags_of_words(image_paths):
    ...
    length = vocab.shape[0]
    res = np.zeros((len(image_paths), length))
    for image_path in image_paths:
        feature_vector = skimage.feature.hog(imread(image_path), feature_vector=True, pixels_per_cell=(4, 4),
                                             cells_per_block=(2, 2))
        feature_vector = feature_vector.reshape(-1, 2 * 2 * 9)
        dist = cdist(feature_vector, vocab)
        closest_vocab = np.argsort(dist, axis=1)[:, 0]
        ...
        res[image_paths.index(image_path)] = histogram
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    vlabels = np.vectorize(lambda x: train_labels[x])
    labels = vlabels(knns)
